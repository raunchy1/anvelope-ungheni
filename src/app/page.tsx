...
export default function Home() {
  const [stats, setStats] = useState({ fise: 0, produse: 0, clienti: 0 });
      ...
      <div className="fade-in" style={{ maxWidth: 980, paddingBottom: 48 }}>
        ...
        <div className="stats-grid" style={{ marginBottom: 44 }}>
          {isLoading ? (
            <>
              {[0,1,2,3].map(i => (
                <div key={i} className="stat-card-new" style={{ '--accent-color': '#3b82f6' } as React.CSSProperties}>
                  <div style={{ height: 38, width: 38, borderRadius: 9, background: 'var(--surface-3)', marginBottom: 14 }} />
                  <div style={{ height: 28, width: 60, borderRadius: 6, background: 'var(--surface-3)', marginBottom: 8 }} />
                  <div style={{ height: 12, width: 100, borderRadius: 4, background: 'var(--surface-3)' }} />
                </div>
              ))}
            </>
          ) : (
            <>
              ...
              <StatCard icon={TrendingUp} label="Uptime" value={100} accent="#f59e0b" suffix="%" />
            </>
          )}
        </div>
